...
import Gear from "@/public/assets/icons/Gear"
import InvoiceLogo from "@/public/assets/icons/InvoiceLogo"
...
export default function CreateInvoiceInPayments() {
  ...
  useEffect(() => {
    if (window !== undefined) {
      ...
      if (paymentExistingPropertyStatus) {
        ...
        // const unitIds = existingPropertyData?.unitId
        const existingUnitIds =
          existingPropertyData &&
          Object.keys(existingPropertyData)
            .filter((key) => key.startsWith("unit"))
            .map((key) => existingPropertyData[key])
        console.log(existingUnitIds) // Assuming the input names start with 'Vendor'
        const ids = existingUnitIds.map((unitItems) => unitItems.unitId)
      ...
      <main className="p-10 relative">
        ...
        <div className="flex flex-col gap-4 items-start right-[43px] top[10px] absolute">
          <button
            ...
            onClick={() =>
              router.push("/dashboard/manager/payments/preview-invoice")
            }
          >
            <Send />
          ...
          <div className="flex w-full items-start justify-between">
            <div className="">
              ...
              <p className="mb-2 text-[10px] capitalize font-[500] ">
                UnitID:{" "}
                {unitIds?.map((element) => (
                  <span key={element.unitId}>{element.unitId}, </span>
                ))}
              </p>
              <p className="mb-2 text-[10px] capitalize font-[500] ">
